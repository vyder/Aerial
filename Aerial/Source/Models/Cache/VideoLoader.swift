    ...
    func connection(_ connection: NSURLConnection, didReceive data: Data) {
        ...
        queue.async { () -> Void in
            ...
            if loadedLocation >= requestedRange.location {
                ...
                let requestedEndOffset = Int(dataRequest.requestedOffset) + Int(dataRequest.requestedLength)
...
//                debugLog("Responding with data")
                ...
                if inset > 0 {
                    ...
                    if dataRequest.currentOffset >= Int64(dataRequest.requestedOffset) + Int64(dataRequest.requestedLength) {
                        self.loadingRequest.finishLoading()
                        self.connection?.cancel()
                    }
                } else if inset < 1 {
